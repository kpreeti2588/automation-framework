...
//	  getAccountId(accessToken);
  ...
  public JSONObject selltickets(String[] ticketId) throws JSONException, IOException, Exception {
	  String accessToken = getAccessToken(emailaddress, password);
	  getAccountId(accessToken);
	  ...
	  for(int i = 0 ; i < ticketId.length; i++) {
		  ...
		  result = post(host + "/api/v1/member/" + Dictionary.get("member_id") + "/posting", accessToken, payload);
	  }
	  String state = waitForTicketState(ticketId[0], "pending");
	  Assert.assertEquals(state, "pending");
	  return result;
  ...
  public class Section implements Comparable<Section>{
	  ...
	  Section(String section_number, int ticketsCount, List<Row> rows) {
  ...
  public class Row implements Comparable<Row>{
	  ...
	  Row(String row_name, List<Ticket> tickets) {
  ...
  public class Ticket implements Comparable<Ticket>{
	  ...
	  Ticket(String seat, boolean can_transfer, boolean can_resale, boolean can_render, boolean can_render_file, boolean can_render_barcode, boolean can_render_passbook, boolean can_donate_charity, boolean has_pending_invoice, boolean is_mobile_enabled) {
		  this.seat = seat;
		  ...
		  for(int j = 0; j < sections.length(); j++) {
			  ...
			  for(int k = 0; k < rows.length(); k++) {
				  ...
				  for(int l = 0; l < tickets.length(); l++) {
					  ...
					  Ticket _ticket = new Ticket(jticket.getString("seat"), can_transfer, can_resale, can_render, can_render_file, can_render_barcode, can_render_passbook, can_donate_charity, has_pending_invoice, is_mobile_enabled);
...
//				  Collections.sort(ltickets);
				  Row crow = new Row(row_name, ltickets);
				  lrows.add(crow);	
			  }
//			  Collections.sort(lrows);
			  Section section = new Section(section_name, persectionticketcount, lrows);
...
//		  Collections.sort(lsections);
  ...
  public Boolean[] getTicketFlags(String ticketId, String email, String pass, boolean... needAccessToken) throws JSONException, IOException {
	          ...
	          for(int l = 0; l < tickets.length(); l++) {
	            ...
	            switch(flag.trim().toUpperCase()) {
	              ...
	              case "CANNOT_RENDER_FILES":
		                if(jticket.has("can_render_file") && !jticket.getBoolean("can_render_file") && (!jticket.has("pending_action") || (jticket.has("pending_action") && jticket.getJSONObject("pending_action").getString("type").trim().equalsIgnoreCase("charity"))))
		                  status = true;
		                break;
  ...
  public List<String> getEventNames(String AcctId) throws Exception {
	  ...
	  try {
		  JSONObject jsonObject = get(host + "/api/v1/member/" + AcctId + "/inventory/events", atoken);
	      JSONArray events =(JSONArray) jsonObject.get("events");
	      
	      if (events != null) { 
	    	  for (int i=0; i<events.length(); i++) { 
//	    		  int eventId = events.getJSONObject(i).getInt("event_id");
	    		  eventName.add(events.getJSONObject(i).getString("name"));
//	    		  System.out.println("event Id :: " + eventId);
//	    		  try{
//		    		  Event _event = getSections(eventId, eventName, atoken);
//		    		  eventsDetail.put(eventId, _event);
...
//	    		  }
  ...
  public String[] renderTicketDetails(String ticket) throws Exception {
	  ...
	  if(jsonObject.has("ticket_render_data_line_items")) {
		  ...
		  String seatLocator = json.getString("entry_gate_label").trim() + " " + json.getString("entry_gate").trim();
