...
public class TicketSteps {
	...
	public TicketSteps(ManageTicket ticket, org.iomedia.framework.Assert Assert, ManageticketsAPI api, BaseUtil base, AccessToken accessToken, Utils utils, org.iomedia.framework.SoftAssert SoftAssert, org.iomedia.framework.Reporting Reporter) {
		this.ticket = ticket;
		this.Assert = Assert;
		this.api = api;
		this.base = base;
	...
	public void verify_ticket_is_displayed(String eventId, String ticket) {
		...
		if(val.length > 2) {
			...
			Assert.assertTrue(this.ticket.isTicketDisplayed(eventId, section, row, seat), "Verify ticket is displayed - " + ticket);
		} else {
	...
	public void user_click_on_manage_tickets(String ticketId ) {
	    ...
	    ticket.clickManageTickets(tkt[0], tkt[1].replaceAll("%20", " "), tkt[2], tkt[3]);
	}
	...
	public void verify_reclaim_ticket_status(String status, String ticketId) throws Exception {
		ticketId = (String) base.getGDValue(ticketId);
		String[] tkt = ticketId.split("\\.");
		Assert.assertEquals(ticket.getTicketStatus(tkt[0], tkt[1], tkt[2], tkt[3]), status);
	}
	...
	public void user_clicked_on_event(String eventname) {
	...
	public void verify_event_details(String ticketId) {
		...
		Assert.assertTrue(ticket.getSection().contains(tkt[1].replaceAll("%20", " ")));
...
//		Assert.assertTrue(ticket.getSeat().contains(tkt[3]));
	...
	public void save_status_of_ticket(String ticketId) {
		ticketId = (String) base.getGDValue(ticketId);
		String[] tkt = ticketId.split("\\.");
		String status = ticket.getTicketStatus(tkt[0], tkt[1].replaceAll("%20", " "), tkt[2], tkt[3]);
...
//		Assert.assertTrue(status.trim().contains(transferName));
	...
	public void verify_user_edit_seller_profile(String email, String pass) throws Exception {
	 ...
	 public void user_click_edit_cancel_ticket(String ticketId) {
		 ticketId = (String) base.getGDValue(ticketId);
		 String[] tkt=ticketId.split("\\.");
		 ticket.clickEditCancelTicket(tkt[0], tkt[1].replaceAll("%20", " "), tkt[2], tkt[3]);
	 }
	 ...
	 public void verify_user_click_Scan_barcode(String ticketId) {
		 ticketId = (String) base.getGDValue(ticketId);
		 String[] tkt = ticketId.split("\\.");
		 ticket.clickScanBarcode(tkt[0], tkt[1].replaceAll("%20", " "), tkt[2], tkt[3]);
...
//		 Assert.assertEquals(ticket.getMobileScanGateNumber(), "Enter Gate: " + base.Dictionary.get("entryGate").trim());
	 ...
	 public void save_barcode_image_into_destination_folder(String BarcodeURL, String ImageFolder) throws Exception {
		...
		String ImagePath = ImageFolder+ OSValidator.delimiter + clientName+"_"+sStartTime+".jpg";
	 ...
	 public void verify_user_claim_ticket(String ticketId,String customer_name) {
		 customer_name = (String) base.getGDValue(customer_name);
		 ticketId = (String) base.getGDValue(ticketId);
		 String[] tkt=ticketId.split("\\.");
		 SoftAssert.assertEquals(ticket.getTicketStatus(tkt[0], tkt[1].replaceAll("%20", " "), tkt[2], tkt[3]), "Claimed by " + customer_name, "Verify claimed text on tickets");
	 }
