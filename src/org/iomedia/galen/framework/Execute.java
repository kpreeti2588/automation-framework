...
public class Execute extends Driver {
	...
	public static void main(String[] args) throws JSONException, Exception{
	    ...
	    if(!number_of_accounts.trim().equalsIgnoreCase("")) {
		    ...
		    create.create(null, createAccount.trim().equalsIgnoreCase("createonly"), Datasheet, true, createAccount.trim().equalsIgnoreCase("addEventsOnlyForNewAccounts"));
	    	return;
	    }
	    ...
	    if(appCredentials.size() > 0) {
		    CreateAccounts create = new CreateAccounts(exec.Environment);
		    appCredentials = create.create(appCredentials, createAccount.trim().equalsIgnoreCase("createonly"), Datasheet, true, createAccount.trim().equalsIgnoreCase("addEventsOnlyForNewAccounts"));
	    ...
	    if(setSession) {
		    ...
		    ses.setSessionLimit(sessions, "", true, true);
	    }
...
//	    }
	    ...
	    if(generateNewTestNgFile) {
	    	testngFileName = environment + "-testng-customsuite.xml";
	    } else {
	...
	public HashMapNew GetXMLNodeValueFromString(String str, String parentNode, int index){
		if(!str.trim().toUpperCase().contains("<CONFIG>")) {
