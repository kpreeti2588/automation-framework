...
public class SwitchAccount extends Driver {
	...
	public void verifyInvoicesSecondarydAccount() throws Exception {
		...
		if(Dictionary.get("SWITCH_ACC_EMAIL_ADDRESS").trim().equalsIgnoreCase(""))
			throw new SkipException("Skipped");
		accessToken.getMemberResponse(Dictionary.get("SWITCH_ACC_EMAIL_ADDRESS"), Dictionary.get("SWITCH_ACC_PASSWORD"));
		...
		if(Dictionary.get("name0").trim().equalsIgnoreCase(Dictionary.get("accId0"))) {
			Assert.assertEquals(header.getSwitchedAccountName(Dictionary.get("CUST_NAME")),Dictionary.get("CUST_NAME"));
		} else {
	...
	public void verifyCardInfoSwitchAccount() throws Exception {
		...
		adminlogin.getCCQueryFieldValues(Dictionary.get(invoicestatus.trim().toUpperCase() + "cc_Query"),new String[]{"cc_name_first","cc_name_last","cc_postal_code","cc_address","cc_exp","data_mask"}, invoicestatus) ;
...
//		invoice.verifyInvoiceStatus(Integer.valueOf(Dictionary.get("invoiceId")), "PLAN");
		adminlogin.beforeMtdInvoiceAPI("", false);
		DecimalFormat df = new DecimalFormat("0.00");
		Assert.assertEquals(df.format(Double.valueOf(newDueAmt)), adminlogin.getTotalAmountDue(Dictionary.get("InvoiceListArray")));
//		String invoicestatus = "UNPAID";
//		int invoiceId = adminlogin.getInvoiceListAndId(Dictionary.get("accId2"), invoicestatus);
//		if(invoiceId == -1){
//			invoicestatus = "PARTIALLY PAID";
//			invoiceId = adminlogin.getInvoiceListAndId(Dictionary.get("accId2"), invoicestatus);
//			if(invoiceId == -1){
//				invoicestatus = "PLAN";
//				invoiceId = adminlogin.getInvoiceListAndId(Dictionary.get("accId2"), invoicestatus);
//			}
...
//		}
		...
		Assert.assertTrue(Dictionary.get("cc_Query").contains(cardnum.substring(cardnum.length() - 4,cardnum.length())));
	}
	...
	public void verifyTicketsSecondarydAccount() throws Exception {
		...
		List<String> eventsList = manageticketsapi.getEventNames(Dictionary.get("memberId1"));
		...
		List<String> eventsList1 = manageticketsapi.getEventNames(Dictionary.get("memberId0"));
	...
	public void verifySendTicketsSwitchAccount() throws Exception {
		if(Dictionary.get("SWITCH_ACC_EMAIL_ADDRESS").trim().equalsIgnoreCase(""))
			throw new SkipException("Skipped");
		if(Dictionary.get("SWITCH_ACC_EMAIL_ADDRESS").trim().equalsIgnoreCase(Dictionary.get("ASSOCIATED_ACC_EMAIL_ADDRESS").trim()))
			throw new SkipException("Skipped");
		JSONObject obj = accessToken.getMemberResponse(Dictionary.get("SWITCH_ACC_EMAIL_ADDRESS"), Dictionary.get("SWITCH_ACC_PASSWORD"));
...
//		Assert.assertTrue(managetickets.getSeat().contains(ticket[3]));
	...
	@Test(groups={"smoke","ticketsFunctional","regression", "switchAccount"}, priority=49)
	public void verifySellTicketsSwitchAccount() throws Exception {
		...
		if(managetickets.isContinueEnabled()) {
			...
			managetickets.clickContinue();
			SoftAssert.assertTrue(managetickets.getSellerAddress(Dictionary.get("Add1")).contains(Dictionary.get("FirstName")));
		...
		try {
			...
			Assert.assertEquals(state, "pending");
			SoftAssert.assertEquals(manageticketsapi.getTicketFlags(Tkt, Dictionary.get("ASSOCIATED_ACC_EMAIL_ADDRESS"), Dictionary.get("ASSOCIATED_ACC_PASSWORD"), true), new Boolean[] {false, false, false, false, false, false, false}, "Verify the ticket flags");
		} finally {
	...
	@Test(groups={"smoke","regression","ticketsFunctional","ViewFunctional","prod","switchAccount"}, priority=47)
	...
	public void verifySwitchAccountNoAssociatedAccounts() throws Exception {
		...
		if((driverType.trim().toUpperCase().contains("ANDROID") || driverType.trim().toUpperCase().contains("IOS")) && Environment.get("deviceType").trim().equalsIgnoreCase("phone")) {
			hamburger.clickMobileHamburger();
			Assert.assertFalse(dashboardSection.checkSwitchAccountPresentMobile(), "Switch Account Option Not Available");
		}
		else {
