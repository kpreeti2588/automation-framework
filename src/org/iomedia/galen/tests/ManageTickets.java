...
public class ManageTickets extends Driver {
	...
	public void verifyDownloadTickets() throws Exception {
		...
		Assert.assertTrue(managetickets.getSection().contains(ticket[1].replaceAll("%20", " ")));
		managetickets.clickContinue();
...
//		Assert.assertTrue(managetickets.getSeat().contains(ticket[3]));
		...
		try{
			...
			String status = managetickets.getTicketStatus(ticket[0], ticket[1].replaceAll("%20", " "), ticket[2], ticket[3]);
	...
	public void verifyMultipleSendTickets() throws Exception {
		...
		Assert.assertEquals(managetickets.getPopUpEventDetails(), Dictionary.get("eventName"));
		Assert.assertTrue(managetickets.getSection().contains(ticket[1].replaceAll("%20", " ")));
		Assert.assertTrue(managetickets.getSection().contains(ticket2[1].replaceAll("%20", " ")));
//		Assert.assertTrue(managetickets.getRow().contains(ticket[2]));
//		Assert.assertTrue(managetickets.getSeat().contains(ticket[3]));
...
//		Assert.assertEquals(managetickets.getMultipleSeat(), ticket2[3]);
		...
		try {
			...
			managetickets.logoutNLogin("", "");
			String status = managetickets.getTicketStatus(ticket[0], ticket[1].replaceAll("%20", " "), ticket[2], ticket[3]);
			String status2 = managetickets.getTicketStatus(ticket2[0], ticket2[1].replaceAll("%20", " "), ticket2[2], ticket2[3]);
			Assert.assertTrue(status.trim().contains(transferName));
			Assert.assertEquals(state, "pending");
	...
	public void verifyDonateTickets() throws Exception {
		...
		managetickets.logoutNLogin("", "");
		Assert.assertEquals(managetickets.getTicketStatus(ticket[0], ticket[1].replaceAll("%20", " "), ticket[2], ticket[3]), "Donated");
	...
	public void verifyMultipleDonateTickets() throws Exception {
		...
		managetickets.logoutNLogin("", "");
		Assert.assertEquals(managetickets.getTicketStatus(ticket[0], ticket[1].replaceAll("%20", " "), ticket[2], ticket[3]), "Donated");
		Assert.assertEquals(managetickets.getTicketStatus(ticket2[0], ticket2[1].replaceAll("%20", " "), ticket2[2], ticket2[3]), "Donated");
		Assert.assertEquals(state, "donated");
	...
	public void verifyReclaimTickets() throws Exception {
		...
		try {
			load("/tickets#/"+ticket[0]);
			homepage.login("", "", null, true);
			managetickets.clickManageTickets(ticket[0], ticket[1].replaceAll("%20", " "), ticket[2], ticket[3]);
		...
		Assert.assertEquals(managetickets.getTicketStatus(ticket[0], ticket[1], ticket[2], ticket[3]), "No Status");
	...
	@Test(groups = {"smoke","regression","ticketsFunctional","prod"}, priority = 8)
...
//			managetickets.clickContinue();
			...
			status = managetickets.getTicketStatus(ticket[0], ticket[1].replaceAll("%20", " "), ticket[2], ticket[3]);
			Assert.assertTrue(status.trim().contains(transferName));
...
//		CAN_DONATE_CHARITY = flags[6];
		...
		try {
			load("/tickets#/"+ticket[0]);
			homepage.login("", "", null, true);
			managetickets.clickEditCancelTicket(ticket[0], ticket[1].replaceAll("%20", " "), ticket[2], ticket[3]);
...
//		CAN_DONATE_CHARITY = flags[6];
	...
	public void downloadBarcode() throws Exception {
		...
		String[] Tkts = manageticketsapi.getRenderDetails(true, "event", false, false);
		String Tkt = Tkts[0];
		String[] ticket=Tkt.split("\\.");
		load("/tickets#/"+ticket[0]);
		homepage.login("", "", null, true);
		
		managetickets.clickScanBarcode(ticket[0], ticket[1].replaceAll("%20", " "), ticket[2], ticket[3]);
	...
	public void verifySellMultipleTickets() throws Exception {
		...
		try {
			...
			managetickets.logoutNLogin("", "");
			String status1 = managetickets.getTicketStatus(ticket1[0], ticket1[1].replaceAll("%20", " "), ticket1[2], ticket1[3]);
			String status2 = managetickets.getTicketStatus(ticket2[0], ticket2[1].replaceAll("%20", " "), ticket2[2], ticket2[3]);
	...
	public void verifyEditMultipleSellTickets() throws Exception{
		String[] Tkts = manageticketsapi.getResaleDetails(true, "event", true, false);
		List<String> Tkt = Arrays.asList(Tkts);
		manageticketsapi.selltickets(new String[]{Tkt.get(0), Tkt.get(1)});
		...
		try {
			...
			String state1 = manageticketsapi.waitForTicketState(Tkt.get(0), "pending");
			String state2=	manageticketsapi.waitForTicketState(Tkt.get(1), "pending");
			String[] state = new String[]{state1, state2};
			managetickets.logoutNLogin("", "");
			int i = 0;
			String[] ticket=Tkt.get(i).split("\\.");
			String status = managetickets.getTicketStatus(ticket[0], ticket[1].replaceAll("%20", " "), ticket[2], ticket[3]);
	...
	public void verifyCancelMultipleSellTickets() throws Exception{
		...
		Boolean[] flags1 = manageticketsapi.getTicketFlags(Tkt.get(0), "", "");
		CAN_TRANSFER = flags1[0];
		CAN_RESALE = flags1[1];
		CAN_RENDER = flags1[2];
		CAN_RENDER_FILE = flags1[3];
		CAN_RENDER_BARCODE = flags1[4];
		CAN_RENDER_PASSBOOK = flags1[5];
		CAN_DONATE_CHARITY = flags1[6];
		
		Boolean[] flags2 = manageticketsapi.getTicketFlags(Tkt.get(1), "", "");
		boolean CAN_TRANSFER1 = flags2[0];
		boolean CAN_RESALE1 = flags2[1];
		boolean CAN_RENDER1 = flags2[2];
		boolean CAN_RENDER_FILE1 = flags2[3];
		boolean CAN_RENDER_BARCODE1 = flags2[4];
		boolean CAN_RENDER_PASSBOOK1 = flags2[5];
		boolean CAN_DONATE_CHARITY1 = flags2[6];
		...
		try {
			load("/tickets#/"+ticket1[0]);
			homepage.login("", "", null, true);
			managetickets.clickEditCancelTicket(ticket1[0], ticket1[1].replaceAll("%20", " "), ticket1[2], ticket1[3]);
			managetickets.clickCancelPosting();
			Assert.assertEquals(managetickets.getPopUpEventDetails(), Dictionary.get("eventName"));
			managetickets.clickConfirm();
			Assert.assertEquals(managetickets.getSuccess(), "Success!");
			
			utils.navigateTo("/tickets#/"+ticket2[0]);
			managetickets.clickEditCancelTicket(ticket2[0], ticket2[1].replaceAll("%20", " "), ticket2[2], ticket2[3]);
			managetickets.clickCancelPosting();
			Assert.assertEquals(managetickets.getPopUpEventDetails(), Dictionary.get("eventName"));
			managetickets.clickConfirm();
			Assert.assertEquals(managetickets.getSuccess(), "Success!");
		...
		managetickets.clickDone();
		Assert.assertTrue(managetickets.isTicketsListDisplayed(null) , "Verify tickets listing page is displayed");
		String state1 = manageticketsapi.waitForPendingActionRemoved(Tkt.get(0));
		String state2 = manageticketsapi.waitForPendingActionRemoved(Tkt.get(1));
		Assert.assertNull(state1, "Verify pending action got removed");
		Assert.assertNull(state2, "Verify pending action got removed");
		String status1 = managetickets.getTicketStatus(ticket1[0], ticket1[1].replaceAll("%20", " "), ticket1[2], ticket1[3]);
		String status2 = managetickets.getTicketStatus(ticket2[0], ticket2[1].replaceAll("%20", " "), ticket2[2], ticket2[3]);
		Assert.assertEquals(status1, "No Status");
		Assert.assertEquals(status2, "No Status");
		SoftAssert.assertEquals(manageticketsapi.getTicketFlags(Tkt.get(0), "", ""), new Boolean[] {CAN_TRANSFER, CAN_RESALE, CAN_RENDER, CAN_RENDER_FILE, CAN_RENDER_BARCODE, CAN_RENDER_PASSBOOK, CAN_DONATE_CHARITY}, "Verify the ticket flags");
		SoftAssert.assertEquals(manageticketsapi.getTicketFlags(Tkt.get(1), "", ""), new Boolean[] {CAN_TRANSFER1, CAN_RESALE1, CAN_RENDER1, CAN_RENDER_FILE1, CAN_RENDER_BARCODE1, CAN_RENDER_PASSBOOK1, CAN_DONATE_CHARITY1}, "Verify the ticket flags");
	}
...
//		System.out.println(eventId);
	...
	public void verifySendParkingTicket() throws Exception {
		...
		managetickets.clickContinue();
		Assert.assertEquals(managetickets.getPopUpEventDetails(), Dictionary.get("eventName"));
		Assert.assertTrue(managetickets.getSection().contains(ticket[1].replaceAll("%20", " ")) || managetickets.getSection().contains(Dictionary.get("FORMATTED_SECTION_NAME_" + Tkt[0])));
//		Assert.assertTrue(managetickets.getRow().contains(ticket[2]));
//		Assert.assertTrue(managetickets.getSeat().contains(ticket[3]));
		managetickets.clickContinue();
		String claimLink = managetickets.getClaimLink();
...
//		CAN_DONATE_CHARITY = flags[6];
	...
	public void verifyClaimTickets() throws Exception {
		...
		Assert.assertEquals(managetickets.getTicketStatus(Tkt[0].split("\\.")[0], Tkt[0].split("\\.")[1].replaceAll("%20", " "), Tkt[0].split("\\.")[2],Tkt[0].split("\\.")[3]), "No Status");
		//For old account
		Assert.assertEquals(manageticketsapi.getTicketFlags(Tkt[0], "", ""), new Boolean[] {false, false, false, false, false, false, false}, "Verify the ticket flags");
		//For new account
		...
		SoftAssert.assertEquals(managetickets.getTicketStatus(ticket[0], ticket[1].replaceAll("%20", " "), ticket[2], ticket[3]), "Claimed by " + names.get("CUST_NAME"), "Verify claimed text on tickets");
	}
	...
	public void verifyClaimParkingTicket() throws Exception {
		...
		try {
			Assert.assertTrue(homepage.getLoginMessage().contains("Congratulations"));
			homepage.createaccount(null, true);
			managetickets.clickClaim();
		...
		Assert.assertEquals(managetickets.getTicketStatus(ticketId.split("\\.")[0], ticketId.split("\\.")[1].replaceAll("%20", " "), ticketId.split("\\.")[2],ticketId.split("\\.")[3]), "No Status");
		//For old account
		...
		managetickets.logoutNLogin("", "");
		Map<String, Object> names = manageticketsapi.getCustomerName(Dictionary.get("NEW_EMAIL_ADDRESS").trim(), Dictionary.get("NEW_PASSWORD").trim());
		SoftAssert.assertEquals(managetickets.getTicketStatus(ticket[0], ticket[1].replaceAll("%20", " "), ticket[2], ticket[3]), "Claimed by " + names.get("CUST_NAME"));
	}
...
//		hamburger.clickManageTickets();
	...
	public void verifyTicketDeferredDelivery() throws Exception {
		...
		homepage.login("", "", null, true);
		if((driverType.trim().toUpperCase().contains("ANDROID") || driverType.trim().toUpperCase().contains("IOS")) && Environment.get("deviceType").trim().equalsIgnoreCase("phone")) {
			Assert.assertFalse(managetickets.verifyScanBarcodeState(ticket[0], ticket[1].replaceAll("%20", " "), ticket[2], ticket[3]));
		}
		else {
